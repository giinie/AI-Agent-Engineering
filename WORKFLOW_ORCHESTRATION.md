...
> Overrides whatever delegation defaults the user-scope configuration sets. The mechanism is the user-scope `CLAUDE.md` § Instruction Precedence (Unified), which places project-scope rules at level 2 and skill/MCP defaults at level 4 — so this file wins for delegation policy when active.
