...
> Overrides user-scope OMC `<delegation_rules>` (aggressive delegation). The mechanism is the user-scope `CLAUDE.md` § Precedence (Canonical), which places project-scope rules at level 2 and OMC defaults at level 4 — so this file wins for delegation policy when active.
